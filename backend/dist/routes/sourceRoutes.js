...
router.post("/", async (req, res) => {
    try {
        ...
        const src = new Source_1.default({ name, availableData, buyingPrice, enrichmentPrice, sellingPrice });
        await src.save();
...
router.put("/:id", async (req, res) => {
    try {
        const { id } = req.params;
        const { name, availableData, buyingPrice, enrichmentPrice, sellingPrice } = req.body;
        const updatedSource = await Source_1.default.findByIdAndUpdate(id, { name, availableData, buyingPrice, enrichmentPrice, sellingPrice }, { new: true, runValidators: true } // return the updated document and validate
        );
        if (!updatedSource) {
            return res.status(404).json({ error: "Source not found" });
        }
